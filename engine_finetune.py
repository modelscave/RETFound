...
def evaluate(data_loader, model, device, args, epoch, mode, num_class, log_writer):
    ...
    jaccard = jaccard_score(true_onehot, pred_onehot, average='weighted')
    average_precision = average_precision_score(true_onehot, pred_softmax, average='weighted')
    kappa = cohen_kappa_score(true_labels, pred_labels)
    f1 = f1_score(true_labels, pred_labels, zero_division=0, average='binary')
    roc_auc = roc_auc_score(true_onehot, pred_softmax, multi_class='ovr', average='weighted')
    precision = precision_score(true_labels, pred_labels, zero_division=0, average='binary')
    recall = recall_score(true_labels, pred_labels, zero_division=0, average='binary')
