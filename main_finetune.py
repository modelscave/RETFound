...
class FocalLoss(torch.nn.Module):
    def __init__(self, alpha=None, gamma=2.0, reduction='mean'):
        ...
        self.alpha = alpha # This will be our class weights
        self.gamma = gamma
...
def get_args_parser():
    ...
    parser.add_argument("--model_arch", default="", type=str, metavar="MODEL_ARCH",
    ...
    parser.add_argument("--finetune", default="RETFound_mae_natureCFP", type=str, help="Checkpoint id/path (see model rules below)")
    ...
    parser.add_argument("--output_dir", default="./output_dir_mae_2.5_1")
...
def main(args):
    ...
    targets = np.array(dataset_train.targets if hasattr(dataset_train, 'targets') else dataset_train.labels)
    class_counts = np.bincount(targets)
    ...
    criterion = FocalLoss(alpha=alpha, gamma=0).to(device)

    if misc.is_main_process():
        print(f"Focal Loss initialized with alpha: {alpha}")
    ...
    if args.adaptation == "lora":
        ...
        print("[Adaptation] LoRA: Training adapters and head.")
